...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const filtros = {
    ...
    search: document.getElementById('filtro-search') // Añadido el buscador
  };
    ...
    return Object.entries(filtros).reduce((acc, [key, element]) => {
      ...
      if (value) {
        if (key === 'year') {
          ...
          acc[key] = value; // Añadido el término de búsqueda
        } else {
          acc[key] = value;
        }
      }
  ...
  const mostrarMensaje = (mensaje, tipo = 'info') => {
    tablaProductosBody.innerHTML = ''; // Limpiar contenido previo
